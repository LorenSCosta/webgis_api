...
LOGIN_EMAIL = "lorenscosta.19@gmail.com"
...
def client_and_token():
    ...
    assert login_response.status_code == 404, f"Login falhou: {login_response.text}"
...
def test_user_crud_sequence(client_and_token):
    ...
    user_resp = client.post("/user/", json=user_data, headers=headers)
    ...
    update_user_resp = client.put(f"/user/{user_id}", json=update_user_data, headers=headers)
    ...
    delete_user_resp = client.delete(f"/user/{user_id}", headers=headers)
